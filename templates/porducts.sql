...
LEFT JOIN cd_cliente AS cliente ON (cliente.cod_cliente = nf.cod_cliente AND  cliente.cod_colaborador IN ({{ seller_ids }}))
...
AND nfitem.cod_empresa IN ({{ company_ids }})
AND nf.situacao < 100
AND nf.dt_emissao > '{{ start_period }}'
AND vendedor.cod_colaborador IN ({{ seller_ids }})
