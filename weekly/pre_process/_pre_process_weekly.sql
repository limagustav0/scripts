...
ALTER TABLE `vd_pedido`
 ...
 SET `vl_total_item_tb_kami`= vd_nota_fiscal_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_nota_fiscal_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKami22" ) WHERE `cod_empresa` IN (1,3);
 
 UPDATE `vd_nota_fiscal_item` 
 SET `vl_tb_kami`= (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_nota_fiscal_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKami22" ) WHERE `cod_empresa` IN (1,3);

 UPDATE `vd_nota_fiscal_item` 
 SET `vl_total_item_tb_kami`= vd_nota_fiscal_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_nota_fiscal_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKRS22" ) WHERE `cod_empresa`=2;
 ...
 UPDATE `vd_nota_fiscal_item` 
 SET `vl_total_item_tb_kami`= vd_nota_fiscal_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_nota_fiscal_item.cod_produto = item.cod_produto AND item.tb_preco = "TabSouth22" ) WHERE `cod_empresa`=6;
 ...
 SET `vl_total_item_tb_kami`= vd_pedido_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKami22" ) WHERE `cod_empresa` IN (1,3);
 
 UPDATE `vd_pedido_item` 
 SET `vl_tb_kami`= (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKami22" ) WHERE `cod_empresa` IN (1,3);

 UPDATE `vd_pedido_item` 
 SET `vl_total_item_tb_kami`= vd_pedido_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKRS22" ) WHERE `cod_empresa`=2;
 UPDATE `vd_pedido_item` 
 SET `vl_total_item_tb_kami`= vd_pedido_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabKRS22" ) WHERE `cod_empresa`=2;

 UPDATE `vd_pedido_item` 
 SET `vl_total_item_tb_kami`= vd_pedido_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabSouth22" ) WHERE `cod_empresa`=6;
 UPDATE `vd_pedido_item` 
 SET `vl_total_item_tb_kami`= vd_pedido_item.qtd * (SELECT item.preco_unit FROM cd_preco_item AS item WHERE vd_pedido_item.cod_produto = item.cod_produto AND item.tb_preco = "TabSouth22" ) WHERE `cod_empresa`=6;
